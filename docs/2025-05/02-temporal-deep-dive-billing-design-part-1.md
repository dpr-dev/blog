...
— Но погоди, а если у клиента есть активные поездки, по которым висят запущенные транзакции в ожидании? Если он удалит карту, то могут возникнуть проблемы с дальшейшим подтверждением этих транзакций! Поездка будет выполнена, но списание не произойдет. Фактически, мы сознательно закладываем способ фрода в систему??
...
— Тут тонкий момент. Мы боимся, что кто-то захочет просто "поиграться" — типа «закажу, а потом отменю». Поэтому мы хотим до начала поиска машины сделать авторизацию — заблокировать сумму на карте. Без фактического списания.
...
- `TRANSACTIONS` — обработка транзакций: авторизация, списание, отмена, возврат.
...
- `TRANSACTIONS.TYPES` — Система должна поддерживать типы транзакций, указанные в `BR.TRANSACTIONS.TYPES`.
...
📦 **Functional Capability: EXTERNAL_INTEGRATION**
...
— (Продакт) Пока всё просто: считаем транзакцию неуспешной, уведомляем пользователя и отправляем алерт в slack — чтобы команда руками посмотрела, что произошло.
...
- `NFR.OBSERVABILITY.METRICS.CALLS` — Система должна собирать метрики внешних вызовов: количество, длительность, ошибки, ретраи.
